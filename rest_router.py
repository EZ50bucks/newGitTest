...
def get_priority(priority_type, vid=0, route=None):
    ...
    print ("getpriorityfunction")

    if priority_type == PRIORITY_TYPE_ROUTE:        #check inside the routing table to print out
    ...
    if vid or priority_type == PRIORITY_IP_HANDLING: #if VLAN => add VLAN for priorty
        priority += PRIORITY_VLAN_SHIFT

    if priority_type > PRIORITY_STATIC_ROUTING: #priority large => netmask shift (32)
        priority += PRIORITY_NETMASK_SHIFT

    if log_msg is None:     #no log_msg => return priority, only log_msg for routing.
        return priority
    else:
...
def get_priority_type(priority, vid):
    if vid:
        priority -= PRIORITY_VLAN_SHIFT     #have vlan id => just calculate priority
    return priority

    print ("getprioritytype")


class NotFoundError(RyuException):          #create a class for NOTFOUND error
...
class RestRouterAPI(app_manager.RyuApp):    #create rest router API

    OFP_VERSIONS = [ofproto_v1_0.OFP_VERSION,   #support v1,1.2,1.3
                    ofproto_v1_2.OFP_VERSION,
                    ofproto_v1_3.OFP_VERSION]

    _CONTEXTS = {'dpset': dpset.DPSet,
                 'wsgi': WSGIApplication}       #WSGI forward requests to web applications or frameworks

    def __init__(self, *args, **kwargs):
        super(RestRouterAPI, self).__init__(*args, **kwargs)    #initialization

        print ("Init function")


        # logger configure
        RouterController.set_logger(self.logger)        #set logger => my logger

        print ("Configurelogger")
        ...
        # For no vlan data
        path = '/router/{switch_id}'        #path router -- switch ID 3 METHOD GET/POST/DELETE
        print ("depend on the Type of packet => start, set and delete - no VLAN")
        ...
        # For vlan data
        path = '/router/{switch_id}/{vlan_id}'      #if have VLAN => redirect to VLAN data
        print ("depend on the Type of packet => start, set and delete - no VLAN")
    ...
    def datapath_handler(self, ev):
        ...
        print ("decide register router")

    @set_ev_cls(ofp_event.EventOFPPacketIn, MAIN_DISPATCHER)        #Asynchronous messages
    def packet_in_handler(self, ev):                #new packet coming -- handle ev.msg

        RouterController.packet_in_handler(ev.msg)
        print ("packet coming in")

    def _stats_reply_handler(self, ev):             #statistic
        print ("stats reply hand;e")
        msg = ev.msg
        dp = msg.datapath

        if (dp.id not in self.waiters
                or msg.xid not in self.waiters[dp.id]):     # not in queue
            return
        event, msgs = self.waiters[dp.id][msg.xid]
        msgs.append(msg)                                    #append msg

        if ofproto_v1_3.OFP_VERSION == dp.ofproto.OFP_VERSION:      #ver 1.3 OF
    ...
    @set_ev_cls(ofp_event.EventOFPFlowStatsReply, MAIN_DISPATCHER)      #Controller -> switch - read state.
    def stats_reply_handler_v1_0(self, ev):
        print ("v1_0 - handle stats reply")
        self._stats_reply_handler(ev)

    # for OpenFlow version1.2/1.3
    @set_ev_cls(ofp_event.EventOFPStatsReply, MAIN_DISPATCHER)
    def stats_reply_handler_v1_2(self, ev):
        print ("v1_2 - handle stats reply")
        self._stats_reply_handler(ev)
...
def rest_command(func):                 #Template command --- func is the parameters ?!
    def _rest_command(*args, **kwargs):
        print ("define rest command - with different error code")
        try:
            msg = func(*args, **kwargs)
            return Response(content_type='application/json',
                            body=json.dumps(msg))           #response by dump the json message

                            print ("response with function in lib")
        ...
        except NotFoundError as msg:            #not found error -> 404
            status = 404
            details = str(msg)

        msg = {REST_RESULT: REST_NG,            #message include status + detail
               REST_DETAILS: details}
...
class RouterController(ControllerBase):         #create controller class

    _ROUTER_LIST = {}                           #no router list
    _LOGGER = None                              #log = none

    def __init__(self, req, link, data, **config):
        super(RouterController, self).__init__(req, link, data, **config)
        self.waiters = data['waiters']          #waiter

    @classmethod
    def set_logger(cls, logger):                #log infor about the switch
        cls._LOGGER = logger
    ...
    @classmethod
    def register_router(cls, dp):               #register the router
        dpid = {'sw_id': dpid_lib.dpid_to_str(dp.id)}       #datapath ID
        try:
            router = Router(dp, cls._LOGGER)    #Logger
        except OFPUnknownVersion as message:
            cls._LOGGER.error(str(message), extra=dpid)     #return error
            return
        cls._ROUTER_LIST.setdefault(dp.id, router)
        cls._LOGGER.info('Join as router.', extra=dpid)     #ROUTER come in => join

    @classmethod
    def unregister_router(cls, dp):
        if dp.id in cls._ROUTER_LIST:                       #in router list => out => delete
            cls._ROUTER_LIST[dp.id].delete()
            del cls._ROUTER_LIST[dp.id]

            dpid = {'sw_id': dpid_lib.dpid_to_str(dp.id)}
            cls._LOGGER.info('Leave router.', extra=dpid)   #some one leave => print out

    @classmethod
    def packet_in_handler(cls, msg):            #CLASS handle incoming pkt
        dp_id = msg.datapath.id
        if dp_id in cls._ROUTER_LIST:           #in the ROUTER - LIST
            router = cls._ROUTER_LIST[dp_id]    #assign dp_id and handle that message
            router.packet_in_handler(msg)

    # GET /router/{switch_id}
    @rest_command
    def get_data(self, req, switch_id, **_kwargs):
        return self._access_router(switch_id, VLANID_NONE,          #GET coming => passs to access_router
                                   'get_data', req)                 #with parameter
    ...
    def _access_router(self, switch_id, vlan_id, func, req):        #function access_router (func - get_data or set data)
        rest_message = []                           #message = array
        routers = self._get_router(switch_id)       #get router with ID
        try:
            param = req.json if req.body else {}    #set json
        except ValueError:                          #wrong format => error
            raise SyntaxError('invalid syntax %s', req.body)
        for router in routers.values():             #if router in list
            function = getattr(router, func)        #get valua of attribute function in ROUTER
            data = function(vlan_id, param, self.waiters)
            rest_message.append(data)               #append datat get from previous
